...
def get_project_settings() -> dict[str, Any]:
    with Path.open(PROJECT_FILE, "rb") as file:
...
def check_update(
    quiet: bool = typer.Option(
        ...
        "-q",
        "--quiet",
        help="Only show if update is available.",
    ),
) -> None:
    ...
    latest = get_latest_release(
        f"{config['project']['urls']['Repository']}/releases/latest",
    )
...
def install_update(
    force: bool = typer.Option(
        ...
        "-f",
        "--force",
        help="Force install even if on latest version.",
    ),
) -> None:
    config = get_project_settings()
    current = config["project"]["version"]
    ...
    if not force and not Version.parse(latest.version) > Version.parse(
        current,
    ):
        console.print("[green]Already on the latest version.[/green]")
        console.print(
            f"Use [bold]--force[/bold] to reinstall version \
            {latest.version}",
        )
        raise typer.Exit(0)
    ...
    try:
        ...
        console.print(
            f"[green]Successfully updated to version {latest.version}[/green]",
        )
        console.print(f"[dim]Release notes: {latest.release_notes_url}[/dim]")
